...
export const guideType = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'body', type: 'array', group: 'content', of: [
      defineArrayMember({ type: 'block', marks: { annotations: [defineArrayMember({ name: 'link', title: 'Link', type: 'object', fields: [defineField({ name: 'href', type: 'string', validation: (r) => r.required() })] })] } }),
      defineArrayMember({ type: 'image', fields: [{ name: 'alt', type: 'string', validation: (r) => r.required() }] }),
      defineArrayMember({ type: 'guideDecisionMap' }),
      defineArrayMember({ type: 'guideLineup' }),
    ...
    defineField({ name: 'authorName', title: 'Public author name', description: 'Use HeyDart for organizational authorship. Do not use Claire or Ben as public bylines.', type: 'string', group: 'editorial', initialValue: 'HeyDart' }),
    defineField({ name: 'author', type: 'reference', to: [{ type: 'person' }], group: 'editorial' }),
